...
def log_request_info(request=None):
    ...
    if request:
        ...
        endpoint = request.url.path  # API endpoint path

    return method, client_host, user_agent,endpoint



def _extract_context(args, kwargs, inner_func):
    request = next(
        (arg for arg in args if isinstance(arg, Request)),
        next((v for v in kwargs.values() if isinstance(v, Request)), None)
    )
    method, ip, user_agent,endpoint = log_request_info(request)
    ...
    return request, method, ip, user_agent, cls, function_name, file, line,username,endpoint

         

# ContextVar to store the current request per async context
...
def log_entry_exit(func: Callable = None, *, app_name: str = None, use_json: bool = True):
    ...
    def decorator(inner_func: Callable) -> Callable:
        ...
        def wrapper(*args, **kwargs):           
            ...
            request, method, ip, user_agent, cls, function_name, file, line,username, endpoint = _extract_context(args, kwargs, inner_func)
